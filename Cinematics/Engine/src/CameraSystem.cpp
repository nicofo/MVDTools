...
void CameraSystem::update(float delta)
{
	...
	auto& camera_components = ECS.getAllComponents<Camera>();
	for (auto & cam : camera_components) {
		if (!cam.viewtrack.empty() && ECS.getEntity(cam.viewtrack) != -1) {
			auto& trans = ECS.getComponentFromEntity<Transform>(cam.viewtrack);
			cam.lookAt(cam.position, trans.position());
		}
	}
...
{
    if (Game::instance->control_system_.input[GLFW_KEY_4] || ECS.updateCameraTo == ECS.getEntity("camera_track")) {
		ECS.updateCameraTo = -1;
		int track_id = ECS.getEntity("camera_track");
        Entity& track = ECS.entities[track_id];
        SetDefaultCamera(track);
		auto& track_it = ECS.getComponentFromEntity<ViewTrack>(track_id);
		track_it.onComplete = []() {
			int track_id = ECS.getEntity("camera_track2");
			ECS.updateCameraTo = track_id;
		};
        std::cout << "Now i'm using a track camera" << std::endl;
		track_it.active = true;
        trackTest = true;
    }

	if (ECS.updateCameraTo == ECS.getEntity("camera_track2")) {
		ECS.updateCameraTo = -1;
		int track_id = ECS.getEntity("camera_track2");
		Entity& track = ECS.entities[track_id];
		SetDefaultCamera(track);
    ...
    if (Game::instance->control_system_.input[GLFW_KEY_5] || ECS.updateCameraTo == ECS.getEntity("camera_flyover")) {
		ECS.updateCameraTo = -1;
		int flyover_id = ECS.getEntity("camera_flyover");
        Entity& flyover = ECS.entities[flyover_id];
        SetDefaultCamera(flyover);
