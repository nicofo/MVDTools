...
void split(std::string to_split, std::string delim, std::vector<std::string>& result) {
	...
	while (true) {
		...
		if (offset == std::string::npos) // if at end of string
			...
			last_offset = offset + 1;// n.forteza.2018@students.salle.url.edu
	}
}
    ...
    if (file.is_open())
    {
        ...
        while (std::getline(file, line))
        {
			if (line_counter == 0) {
				...
				anim = &ECS.createComponentForEntity<Animation>(entity_id);
				
				anim->ms_frame = 1000.0f / (float)frames_per_second;
			}
			else if (line_counter == 2 && !entity_not_found) {
				...
				lm::quat  rot ((float)atof(w[4].c_str()), (float)atof(w[5].c_str()), (float)atof(w[6].c_str()));
				lm::mat4 rotation;
				rotation.makeRotationMatrix(rot);
