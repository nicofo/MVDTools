...
public class ClickSpawn : MonoBehaviour
{
    ...
    // Setup, delegates and physics settings
    ...
    // Do the raycast with the scene, get desired position and place the item.
    void OnScene(SceneView scene)
    {
        ...
        if (Physics.Raycast(ray, out hit, float.PositiveInfinity, 1 << layerIndex))
        {
            ...
            Handles.DrawDottedLine(hit.point , final_pos, 0.2f);
            ...
            if (e.type == EventType.MouseDown && e.button == 2)
            {
                ...
                go.transform.rotation = new_rotation; go.transform.rotation = Quaternion.Euler(rotation);
